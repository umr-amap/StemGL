...
%
    ...
    if fid > 0
        ...
        labels = fgetl(fid); %read"information"
        %fprintf(1,'\n %s',labels);

        labels = fgetl(fid); %read "  Age_max Nb observ "
        %fprintf(1,'\n%s',labels);

        strvals = fgetl(fid);
        %fprintf(1,'\nlabel_vals %s \n', strvals);
        [vals,nbv] = sscanf(strvals,'%f',2);
        %fprintf(1,' %d ', nbv);
        %fprintf(1,' %f', vals);
        T_T = vals(1);
        ...
        labels = fgetl(fid); % "date_of_measurements"
        %fprintf(1,'\n%s',labels);

        strvals = fgetl(fid); % "dates"
        %fprintf(1,'\n%s\n',strvals);
        [vals,nbv] = sscanf(strvals,'%f',mxTob);
        %fprintf(1,' %d ', nbv);
        %fprintf(1,' %f', vals);
        info_Dat(1,1:mxTob) = vals(1:mxTob);

        labels = fgetl(fid);%organe production
        %fprintf(1,'\n%s',labels);
        ...
        if comp(1,1) >0
            ...
            for k=1:mxTob
                ...
                strvals = fgetl(fid);
                %fprintf(1,'\n obs %d    label_vals %s \n', k, strvals);
                [vals,nbv] = sscanf(strvals,'%f',ncol);
                %fprintf(1,' %d ', nbv);
                %fprintf(1,' %f', vals);
                info(1,1:nbv)= vals(1:nbv);
