...
%
    ...
    plant_wkname = strcat(dirwkparam, filesep, nameplant);
    if (isdir (dirwkparam)) == 0
       mkdir (dirwkparam);
    end
    ...
    if Ti > 0
        ...
        %
        if Msk == 1
            [filename] = IO_FileGUI(strcat (plant_name, '.msk'),'Select Mask file', sdir, '*.msk',0);
...
[~,mxTob] = size(Tob);
    ...
    Draw_Plant(chrono,plant_name,No,Leaf_Display,Internode_Display,Fruit_Display,T,Tm,Axdc,...
    ...
    Write_Target = 1;
    if Write_Target==1
        IO_WriteTarg(strcat(plant_wkname, '.targ'),comp,T,mxTob,Tob,TQAS,TQPS,TQICS,TQFS,TQMS,TQTS,...
    ...
    Write_Mask = 1;
    if Write_Mask == 1
        IO_WriteMsk(strcat(plant_wkname, '.msk'),T,Tm,na,np,ni,nf,nm); % MJ Juste ? j'aurai pris Axdc
    end
